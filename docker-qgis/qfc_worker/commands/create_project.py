...
def get_project_seed(project_id: str) -> ProjectSeedDict:
    client = sdk.Client()
...
def get_project_seed_xlsform(
    project_id: str, project_seed: ProjectSeedDict, destination_dir: str
) -> Path | None:
    if not project_seed["xlsform"]:
...
def _create_project_from_xlsform(
    xlsform_filename: Path | str, project_seed: ProjectSeedDict, tmp_project_dir: str
):
    assert project_seed["xlsform"]
    ...
    converter.set_groups_as_tabs(project_seed["xlsform"]["show_groups_as_tabs"])
...
def create_project_from_seed(
    project_seed: ProjectSeedDict, tmp_project_dir: str, xlsform_filename: str
) -> str:
    ...
    if project_filename:
        ...
        project_filename = Path(tmp_project_dir).joinpath("files", "project.qgs")

    if project_seed["basemaps"]:
        logger.info("Adding basemaps to the project...")

        for basemap_config in project_seed["basemaps"]:
