...
def check_valid_project_file(the_qgis_file_filename: Path) -> None:
    logger.info("Check QGIS project file validity…")

    if not the_qgis_file_filename.exists():
        raise ProjectFileNotFoundException(
            the_qgis_file_filename=the_qgis_file_filename
        )

    if the_qgis_file_filename.suffix == ".qgs":
        with open(the_qgis_file_filename, "rb") as fh:
            try:
    ...
    elif the_qgis_file_filename.suffix != ".qgz":
        raise InvalidFileExtensionException(
            the_qgis_file_filename=the_qgis_file_filename,
            extension=the_qgis_file_filename.suffix,
        )
...
def generate_thumbnail(the_qgis_file_filename: str, thumbnail_filename: Path) -> None:
