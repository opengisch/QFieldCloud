...
def upload_file(token, project_id, local_file, remote_file):
    ...
    _ = cloud_request(
        "POST",
        f"files/{project_id}/{remote_file}",
        token=token,
        files={
            "file": local_file,
        },
...
@click.option(
    "--recursive/--no-recursive", default=True, help="Recursively upload files"
)
...
def export_files_download(token, project_id, remote_file, local_file):
    ...
    resp = cloud_request(
        "GET", f"qfield-files/{project_id}/{remote_file}", token=token, stream=True
    )
...
@click.argument("token", envvar="QFIELDCLOUD_TOKEN", type=str)
@click.option(
    "--accept-old-export/--force-fresh-export",
    default=True,
    help="Accept old exports too",
)
def export(token, project_id, local_dir, accept_old_export):
    ...
    while True:
        resp = cloud_request(
            "GET", f"qfield-files/export/{project_id}", token=token, exit_on_error=False
        )
    ...
    for file in files:
        resp = cloud_request(
            "GET", f'qfield-files/{project_id}/{file["name"]}', token=token, stream=True
        )
...
def cloud_request(
    ...
    try:
        ...
        if exit_on_error:
            is_error_printed = 1
            print(
                f"{response.request.method} {response.url} got HTTP {response.status_code}"
            )
