...
DEFAULT_WHITELABEL = {
    # Branding
    "site_title": "QFieldCloud",  # Translated in the context processor
    # Logos (paths relative to static directory)
    "logo_navbar": "logo_sidetext_white.svg",
    "logo_main": "logo_undertext.svg",
    "logo_alt": "QFieldCloud logo",  # Translated in the context processor
    "favicon": "favicon.ico",
    # Styling
    "custom_css": "css/qfieldcloud.css",
    # Footer
    "footer_text": "Made with 💝 by ",  # Translated in the context processor
    "footer_company_name": "OPENGIS.ch",  # Translated in the context processor
    "footer_company_url": "https://opengis.ch",
}
...
def get_whitelabel_settings() -> dict[str, Any]:
    ...
    # Get user settings, filtering out None values
    user_settings = {
        key: value
        for key, value in getattr(settings, "WHITELABEL", {}).items()
...
def whitelabel(request: HttpRequest) -> dict[str, Any]:
    ...
    whitelabel_config["site_title"] = _(whitelabel_config["site_title"])
    whitelabel_config["logo_alt"] = _(whitelabel_config["logo_alt"])
    whitelabel_config["footer_text"] = _(whitelabel_config["footer_text"])
    whitelabel_config["footer_company_name"] = _(
        whitelabel_config["footer_company_name"]
    )
