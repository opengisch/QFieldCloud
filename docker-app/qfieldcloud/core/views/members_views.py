...
class GetUpdateDestroyMemberViewPermissions(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if request.method == "GET":
            return permissions_utils.can_read_members(user, organization, member)
        if request.method in ["PUT", "PATCH"]:
            return permissions_utils.can_update_members(user, organization, member)
        if request.method in ["DELETE"]:
            return permissions_utils.can_delete_members(user, organization, member)
        return False
