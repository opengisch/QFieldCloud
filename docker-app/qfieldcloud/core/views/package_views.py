...
class LegacyLatestPackageView(views.APIView):
    ...
    def get(self, request, project_id):
        ...
        for f in get_project_package_files(project_id, str(last_package_job.id)):
            file_data = {
                "name": f.name,
                "size": f.size,
        ...
        assert last_package_job.feedback

        feedback_version = last_package_job.feedback.get("feedback_version")

        # version 2 and 3 have the same format
        if feedback_version in ["2.0", "3.0"]:
            layers = last_package_job.feedback["outputs"]["qgis_layers_data"][
                "layers_by_id"
            ]
        # support some ancient QFieldCloud job data
        elif feedback_version is None:
            steps = last_package_job.feedback.get("steps", [])
            layers = (
        ...
        return Response(
            {
                "files": files,
                "layers": layers,
                "status": last_package_job.status,
                "package_id": last_package_job.pk,
                "packaged_at": last_package_job.project.data_last_packaged_at,
                "data_last_updated_at": last_package_job.project.data_last_updated_at,
            }
        )
...
class LegacyLatestPackageDownloadFilesView(views.APIView):
    ...
    def get(self, request, project_id, filename):
        ...
        key = f"projects/{project_id}/packages/{last_package_job.id}/{filename}"
...
class LatestPackageDownloadFilesView(views.APIView):
    ...
    def get(self, request, project_id, filename):
        ...
        return download_project_file_version(
            ...
            filename,
            file_type=file_type,
            package_job_id=last_package_job.id,
        )
