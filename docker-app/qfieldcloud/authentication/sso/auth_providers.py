...
def get_credentials_provider() -> dict[str, str]:
    return {
        "type": "credentials",
        "id": "credentials",
...
def get_auth_providers(request: HttpRequest) -> list[dict]:
    ...
    for provider in social_account_adapter.list_providers(request):
        auth_providers.append(get_oauth2_provider(provider, request))
