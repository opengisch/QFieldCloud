...
def transform_wkt_crs(
    ...
    crs_min_x, crs_min_y, crs_max_x, crs_max_y = transformer.transform_bounds(
        crs_area_of_use.west,
        crs_area_of_use.south,
        crs_area_of_use.east,
        crs_area_of_use.north,
    )

    geom_min_x, geom_min_y, geom_max_x, geom_max_y = geometry.extent
    if not (
        crs_min_x <= geom_min_x <= geom_max_x <= crs_max_x
        and crs_min_y <= geom_min_y <= geom_max_y <= crs_max_y
    ):
        raise ValueError(
            f"Transformed geometry {wkt=} from {source_crs=} to {target_crs=} "
            f"is outside {target_crs}'s area of use: extent={geometry.extent}, "
            f"expected within=({crs_min_x}, {crs_min_y}, {crs_max_x}, {crs_max_y})"
        )
